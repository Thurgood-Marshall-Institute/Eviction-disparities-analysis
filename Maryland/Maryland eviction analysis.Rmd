...
evic <- read_csv("./Data/District_Court_of_Maryland_Eviction_Case_Data.csv")
...
#restrict to evictions in April 2023 or later and group by zip code
#Time period = April - Nov 2023
df1 <- evic %>% filter(`Evicted Date`>"2023-03-31") %>% group_by(`Tenant ZIP Code`) %>% summarize(count=n())
...
#Residuals for negative binomial plot are much smaller, Poisson residuals extend all the way to 20 - suggests that negative binomial is a better fit than Poisson

pchisq(2 * (logLik(nb_model) - logLik(p_model)), df = 1, lower.tail = FALSE)

#Negative binomial offers significantly better fit
```
<iframe title="Rate of executed evictions by ZIP code (Apr - Nov 2023)" aria-label="Map" id="datawrapper-chart-9V6vk" src="https://datawrapper.dwcdn.net/9V6vk/1/" scrolling="no" frameborder="0" style="width: 0; min-width: 100% !important; border: none;" height="459" data-external="1"></iframe><script type="text/javascript">!function(){"use strict";window.addEventListener("message",(function(a){if(void 0!==a.data["datawrapper-height"]){var e=document.querySelectorAll("iframe");for(var t in a.data["datawrapper-height"])for(var r=0;r<e.length;r++)if(e[r].contentWindow===a.source){var i=a.data["datawrapper-height"][t]+"px";e[r].style.height=i}}}))}();
</script>
...
A 10-percentage-point increase in the percent of Black renters in a ZIP code is associated with a 21% increase in the rate of evictions.

## Executed holdover evictions
```{r holdover eviction data, echo=FALSE, warning=FALSE, message=FALSE}
#restrict to evictions in April 2023 or later, restrict to holdover evics, and group by zip code
#Time period = April - Nov 2023
df2 <- evic %>% filter(`Evicted Date`>"2023-03-31" & `Case Type`=="Tenant Holding Over") %>% group_by(`Tenant ZIP Code`) %>% summarize(count=n())
...
A 10-percentage-point increase in the percent of Black renters in a ZIP code is associated with a 22% increase in the rate of holdover evictions.

# Warrants of restitution
## Total warrants of restitution

```{r warrant data, echo=FALSE, warning=FALSE, message=FALSE}
#restrict to warrants in July 2023 or later and group by zip code
#Time period = July - Nov 2023
df3 <- evic %>% filter(`Event Date`>"2023-06-30") %>% group_by(`Tenant ZIP Code`) %>% summarize(count=n())
...
#Residuals for negative binomial plot are smaller, Poisson residuals extend all the way to 100 - suggests that negative binomial is a better fit than Poisson
...
#restrict to warrants in July 2023 or later, restrict to holdover, and group by zip code
#Time period = July - Nov 2023
df4 <- evic %>% filter(`Event Date`>"2023-06-30" & `Case Type`=="Tenant Holding Over") %>% group_by(`Tenant ZIP Code`) %>% summarize(count=n())
...
A 10-percentage-point increase in the percent of Black renters in a ZIP code is associated with a 29% increase in the rate of filings of warrants of restitution for holdover evictions.

```{r exports datasets for maps, echo=FALSE, warning=FALSE, message=FALSE}
write.csv(df1, "./Map Data/total executed evictions.csv")
write.csv(df2, "./Map Data/executed holdover evictions.csv")
write.csv(df3, "./Map Data/warrants of restitution.csv")
write.csv(df4, "./Map Data/holdover warrants.csv")
